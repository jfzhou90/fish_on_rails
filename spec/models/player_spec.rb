...
describe(Player) do
  ...
  let(:player_json) { existing_player.as_json }
  let(:inflated_player) { Player.from_json(player_json) }
  ...
  describe('#as_json') do
    it('creates a json object') do
      expect(player_json['name']).to eq('Joey')
    end
  end

  describe('#from_json') do
    it('creates player class from json') do
      expect(inflated_player).to be_instance_of(Player)
    end

    it('inflated player have the correct attributes') do
      expect(inflated_player.points).to be(1)
      expect(inflated_player.name).to eq('Joey')
      expect(inflated_player.hand[0]).to be_instance_of(PlayingCard)
    end
  end
end
